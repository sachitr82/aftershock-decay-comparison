...
You should give attribution to the academic books and articles that you draw from and build upon in your research report. You must give full attribution if and how you used generational AI during your research, referencing its use as outlined in our Guidance to using generational AI on the MSc in Statistics. 

You should use inline or parenthetical references to your sources. Inline references are used when a source is used as a noun within your sentence. For example, \cite{efrontib} describes the bootstrap procedure. Parenthetical references are used to support a claim where the source is not explicitly mentioned within the sentence. For example, the bootstrap procedure stemmed from an earlier approach called the jackknife, which withholds a single data entry at each iteration \citep{efron1979}.
...
You can also cite works that cannot be recovered directly by the reader. This is cited as a \textit{personal communication}, which historically would have represented an in-person conversation or letter exchange. This now extends to many other forms of irrecoverable exchange including: email or instant messaging, interviews, live speaches, meetings and phone calls or notes made during a lecture that were not formally distributed.

You should use a personal communication citation only when a recoverable source is not available and when the communication or work contains original content that is not readily available in any other source. For example, the proprietary models currently used rely strongly on the assumption of independent observations, despite it leading to overly confident parameter estimates \citep{alexander2024}. 

To reference the output of a large language model using BibTeX, you can follow a format similar to citing software or personal communication. Here's a recommended format based on academic conventions as of 2025:

\begin{verbatim}
@misc{chatgpt2025a,
  author       = {{OpenAI}},
  title        = {{ChatGPT response to a user query: how should I 
                  reference this query using BibTeX?}},
  year         = {2025},
...
This can then be referenced within the main text \citep{chatgpt2025a}.
