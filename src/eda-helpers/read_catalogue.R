...
#'
#' Reads a raw file from the SCEDC catalogue search ("SCEDC" output format), 
#' discards non-data lines, names columns, and returns a tidy data frame with 
#' parsed times and convenience columns for exploratory analysis.
#' 
#' @details
...
#'   (`datetime` in `tz`), the UTC calendar features `year`, `month`, and
#'   `month_start`, and the local-time features `hour` and `wday`
#'   (`month` and `wday` are ordered factors).
...
read_catalogue <- function(path, tz = "America/Los_Angeles",
  ...
  # Convert filtered text into a data frame, allowing incomplete rows
  ...
  # datetime and calendar features stay in UTC; local-time features are derived
  # in `tz`.
  cat <- cat[order(cat$datetime), , drop = FALSE] 
  cat$event_num <- seq_len(nrow(cat))   
  cat$local <- with_tz(cat$datetime, tz) # local
